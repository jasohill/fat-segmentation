...
if subject < 10
    ...
    interp_fn = ['subject0',num2str(subject),'_',letter,'_IVDinterp.mat'];
    interp_fn_other = ['subject0',num2str(subject),'_',letterOther,'_IVDinterp.mat'];    
else
...
scanOffsetOther  = round(IVD.scanOffset);
...
SCATvolume   = translate_lower(SCATvolume,length(upperSlices),IVD.scanOffset - scanOffset);
VATvolume    = translate_lower(VATvolume,length(upperSlices),IVD.scanOffset - scanOffset);
IMATvolume   = translate_lower(IMATvolume,length(upperSlices),IVD.scanOffset - scanOffset);
organsVolume = translate_lower(organsVolume,length(upperSlices),IVD.scanOffset - scanOffset);
voidsVolume  = translate_lower(voidsVolume,length(upperSlices),IVD.scanOffset - scanOffset);
for j = 1:size(Isaggital,2)
Isaggital(:,j)  = translate_lower(Isaggital(:,j),length(upperSlices),IVD.scanOffset - scanOffset);
end
...
for j = 1:Noverlap
    ...
    Isaggital(overlap_upper_i(j),:)  = average_weight*Isaggital(overlap_lower_i(j),:)  + (1-average_weight)*Isaggital(overlap_upper_i(j),:); 
end
...
disp(X)
if write_results
A = [SCATthorax SCATabNoClip SCATabClipped SCATest]; %#ok<*UNRCH>
